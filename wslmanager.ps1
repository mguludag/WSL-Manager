...
function mainfn {
	do {
		...
		Write-Host "------------------------------------------------------------"
		((Get-ChildItem HKCU:\Software\Microsoft\Windows\CurrentVersion\Lxss | ForEach-Object { Get-ItemProperty $_.PSPath }) | Select-Object @{n = "Index"; e = { (++$Global:index) } }, @{n = "DistributionName"; e = { $_.DistributionName; ($Global:name += $_.DistributionName)>$null } }, @{n = "Version"; e = { $_.Version; ($Global:version += $_.Version)>$null } }, @{n = "Size"; e = { ((Get-ChildItem ($_.BasePath.Replace("\\?\", "").Replace("\RootFS", "")+"\ext4.vhdx")).Length / 1GB).ToString().substring(0, 4)+"GB"; ($Global:size += ((Get-ChildItem ($_.BasePath.Replace("\\?\", "").Replace("\RootFS", "")+"\ext4.vhdx")).Length / 1GB))>$null } }, @{n = "Path"; e = { $_.BasePath.Replace("\\?\", "").Replace("\RootFS", ""); ($Global:path += $_.BasePath.Replace("\\?\", "").Replace("\RootFS", ""))>$null } }) | Out-String
		Write-Host "----------------------------------------------------------------------------------------------------------------"
...
function change_version {
	$idx = Read-Host "Select Index number to change distro wsl version(0 - $index)"
	if (($idx -ge 0) -and ($idx -le $index)) {
